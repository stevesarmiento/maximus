...
export function PerlinContoursBackground() {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      const contourLevels = [0.1, 0.35, 0.5, 0.65, 0.8];
      const scale = 0.01;
      ...
      contourLevels.forEach((level, levelIndex) => {
        ...
        for (let x = 0; x < canvas.width; x += 4) {
          for (let y = 0; y < canvas.height; y += 4) {
            const noiseValue = (noiseRef.current.perlin2(
              x * scale + timeRef.current,
              y * scale
            ) + 1) / 2;
            
            if (Math.abs(noiseValue - level) < 0.05) {
              points.push({ x, y });
            }
          }
        }
        ...
        if (points.length > 0) {
          ctx.strokeStyle = `rgba(255, 224, 194, ${0.15 + levelIndex * 0.03})`;
          ctx.lineWidth = 1;
          ctx.setLineDash([2, 4]);
          ...
          points.forEach(point => {
            if (Math.random() > 0.7) {
              ctx.fillStyle = `rgba(255, 224, 194, ${1 + levelIndex * 0.02})`;
              ctx.beginPath();
